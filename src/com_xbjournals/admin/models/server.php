 ...
 * @filesource admin/models/server.php
...
class XbjournalsModelServer extends JModelAdmin {
    ...
    public $typeAlias = 'com_xbjournals.server';
...
//             $item->tags = $tagsHelper->getTagIds($item->id, 'com_xbmaps.map');
    ...
    public function getForm($data = array(), $loadData = true) {
        ...
        $form = $this->loadForm( 'com_xbjournals.server', 'server',
    ...
    protected function loadFormData() {
        $data = Factory::getApplication()->getUserState('com_xbjournals.edit.server.data', array() );
    ...
    protected function prepareTable($table) {
        ...
        if (empty($table->id)) {
            ...
            if (empty($table->ordering)) {
                ...
                ->select('MAX(ordering)')
                ->from($db->quoteName('#__xbjournals_servers'));
    ...
    public function publish(&$pks, $value = 1) {
        if (!empty($pks)) {
            foreach ($pks as $item) {
                $db = $this->getDbo();
                $query = $db->getQuery(true)
                ->update($db->quoteName('#__xbjournals_servers'))
                ->set('state = ' . (int) $value)
                ->where('id='.$item);
                $db->setQuery($query);
                if (!($db->execute())) {
